...
export default function NotificationsMenu({ rattrapages }) {
  const filteredRattrapages = rattrapages.filter(
    (rattrapage) => rattrapage.is_accepted || rattrapage.is_declined
  );
  ...
  return (
    <Menu>
      ...
      <MenuList className="flex flex-col gap-2">
        {filteredRattrapages.length > 0 ? (
          filteredRattrapages.map((rattrapage, index) => {
            const courseName = rattrapage.nom_cour;
            const isAccepted = rattrapage.is_accepted;
            const isDeclined = rattrapage.is_declined;

            
            const statusMessage = isAccepted
              ? `The educator has accepted your catchup course for ${courseName}`
              : isDeclined
              ? `The educator has declined your catchup course for ${courseName}`
              : "";
              
            const messageStyle = isAccepted
            ...
            return (
              <MenuItem
                key={index}
                className={`flex items-center gap-4 py-2 pl-2 pr-8 ${messageStyle}`}
              >
                <div className="flex flex-col gap-1">
                  ...
                  <Typography className="flex items-center gap-1 text-sm font-medium text-blue-gray-500">
                    <ClockIcon />
                    {formatDistanceToNow(new Date(rattrapage.updated_at), {
                      addSuffix: true,
                    })}
                  </Typography>
                </div>
          ...
          <MenuItem className="py-2">
            ...
            >
              No catchup courses have been accepted or declined yet.
            </Typography>
          </MenuItem>
        )}
      </MenuList>
